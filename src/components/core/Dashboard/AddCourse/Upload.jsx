...
export default function Upload({
  ...
  const onDrop = (acceptedFiles) => {
    ...
    if (file) {
      previewFile(file);
      setSelectedFile(file);
      setValue(name, file); // Store the file in form state
    }
  };
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    accept: video
      ? { "video/mp4": [] } // Correct format for videos
      : { "image/jpeg": [], "image/png": [], "image/jpg": [] }, // Correct format for images
    onDrop,
  });
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <div
        ...
        onClick={handleClick} // Ensures clicking works
      >
        <input {...getInputProps()} ref={inputRef} />
